/**
 * Seed rating records (Stage 5 behavior-based trust data).
 *
 * Each record is a review submitted by a client after a completed job.
 * Shape: { id, workerId, jobId, clientName, stars, comment, categories, submittedAt, flags }
 *   categories: { punctuality, quality, communication } on a 1-5 scale
 *   flags: optional { noShow: boolean, complaint: boolean } for negative signals
 *
 * `jobId` references an entry in src/data/jobs.js when the review came from
 * a platform-tracked completion. Older jobs from a worker's pre-platform
 * history carry `jobId: null` and a free-form `clientName` string.
 *
 * The RatingsContext seeds localStorage from this list on first load, then
 * treats localStorage as the source of truth so new ratings persist.
 */
